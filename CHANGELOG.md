...
- **Origins 5.5e — a third game, and a character builder for it.**
  [Origins 5.5e](https://patchworkpaladin.com/2026/08/30/origins-5-5e/) is
  Patchwork Paladin's classless, level-less hack of SRD 5.2.1: you build a
  character out of the character-origin rules alone — a background, a species,
  ability scores — and borrow a "reference class" for its Hit Point Die, Armor
  Training, Shields and Weapon Proficiencies and _nothing else_, which means no
  saving-throw proficiencies, no class features, and a proficiency bonus fixed
  at +2. It lives at `/origins`: seven wizard steps, a printable sheet, and a
  play mode with the once-per-Long-Rest spell bookkeeping the hack uses in place
  of spell slots. Deliberately builder-only — no rules reference, no GM guide,
  no campaign surface.

  The pack is generated by a new `tools/build_origins_data.py`, which extracts
  the SRD's four backgrounds, nine species, twelve classes' core traits, four
  Origin feats, equipment, and the cantrip/level-1 slice of the Cleric, Druid
  and Wizard spell lists. Both bodies of text are CC BY 4.0 — Origins 5.5e
  © 2026 Patchwork Paladin, SRD 5.2.1 © 2024 Wizards of the Coast LLC — so the
  pack declares one licence and carries both credits, with the SRD's required
  notice verbatim.

  **The framework promise held for the third time, and this was the first game
  to need no shell change at all** — notable because it was also the hardest
  case: Stonetop and HMtW are both PbtA-shaped, while Origins is a d20 game
  whose character creation has nothing in common with a playbook. The generic
  wizard, the choices-so-far rail, the sheet/play slots and the entity-type map
  took it as they were.
