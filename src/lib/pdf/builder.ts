...
export interface EmbeddedFont {
	font: PDFFont;
	measure: MeasureFn;
	/** Make a string drawable in this font (see `sanitizeForFont`). */
...
/**
 * The 14 standard fonts encode WinAnsi only — a stray ◇ or ✕ from pack text
 * would throw mid-draw. Map the symbols game text actually uses to printable
 * stand-ins, then flatten anything else non-encodable to '?' (for standard
 * fonts) rather than turning a download into a 500. Embedded fonts keep the
 * symbol map but skip the flatten — fontkit can encode whatever glyphs the
 * font carries.
 */
function sanitizeForFont(text: string, standard: boolean): string {
	const mapped = text
		.replace(/[✕✗✘×]/g, 'x')
		.replace(/[✓✔]/g, 'x')
		.replace(/[◇◆□■]/g, '( )')
		.replace(/[●•◦]/g, '-');
	if (!standard) return mapped;
	// Keep: newline (the wrap/split path needs it), printable ASCII, no-break
	// space, latin-1 proper (¡–ÿ — deliberately *not* U+0080–U+009F, which are
	// C1 controls pdf-lib refuses), and the typographic extras WinAnsi carries
	// that pack text actually uses. Everything else flattens to '?'. The
...
export class PdfBuilder {
	...
	private wrap(font: PDFFont, standard: boolean): EmbeddedFont {
		return {
			font,
			sanitize: (text) => sanitizeForFont(text, standard),
			measure: (text, size) => font.widthOfTextAtSize(sanitizeForFont(text, standard), size),
	...
	/** How many lines a text run will wrap to — for measuring a block's height
	 * before drawing it (keep-together decisions live with the caller). */
	measureLines(text: string, font: EmbeddedFont, size: number, maxWidth: number): number {
		return wrapText(text, maxWidth, size, font.measure).length;
	}
